...
func FuzzyMatchV2(
	...
	pidx, lastIdx := 0, 0
	pchar0, pchar, prevH0, prevClass, inGap := pattern[0], pattern[0], int16(
		0,
	), initialCharClass, false
	Tsub := T[idx:]
	...
	for off, char := range Tsub {
		...
		if char == pchar {
			if pidx < M {
				...
				pidx++
				pchar = pattern[geom.Min(pidx, M-1)]
			}
			lastIdx = idx + off
		}
		...
		if char == pchar0 {
			...
			if inGap {
				H0sub[off] = geom.Max16(prevH0+scoreGapExtension, 0)
			} else {
				H0sub[off] = geom.Max16(prevH0+scoreGapStart, 0)
			}
			C0sub[off] = 0
			inGap = true
		}
	...
	for off, f := range Fsub {
		...
		for off, char := range Tsub {
			...
			if pchar == char {
				...
				if consecutive > 1 {
					...
					if b >= bonusBoundary && b > fb {
						...
						b = geom.Max16(b, geom.Max16(bonusConsecutive, fb))
					}
				}
				if s1+b < s2 {
					s1 += Bsub[off]
			...
			inGap = s1 < s2
			score := geom.Max16(geom.Max16(s1, s2), 0)
			if pidx == M-1 &&
				(forward && score > maxScore || !forward && score >= maxScore) {
	...
	if withPos {
		...
		for {
			...
			preferMatch = C[I+j0] > 1 ||
				I+width+j0+1 < len(C) && C[I+width+j0+1] > 0
			j--
		}
	}
	...
	for idx := sidx; idx < eidx; idx++ {
		...
		if char == pattern[pidx] {
			...
			if consecutive == 0 {
				...
				bonus = geom.Max16(geom.Max16(bonus, firstBonus), bonusConsecutive)
			}
			if pidx == 0 {
...
func EqualMatch(
	...
	if match {
		return Result{
			trimmedLen,
			trimmedLen + lenPattern,
			(scoreMatch+int(bonusBoundaryWhite))*lenPattern +
				(bonusFirstCharMultiplier-1)*int(bonusBoundaryWhite),
		}, nil
	}
	return Result{-1, -1, 0}, nil
}
