...
Signal all goroutines waiting on the named channel `name`. Any call to {{api signal/wait}} with the same name will unblock. If no one is waiting, the signal is silently dropped.
...
Block until the named channel `name` is signaled via {{api signal/send}}. Multiple goroutines can wait on the same channel; all will be woken when the channel is signaled.
