...
Bind the key sequence `sequence` to `callback` for node `target`, which is a [NodeID](/api.md#nodeid), `:time` (for [time mode](/replay-mode/modes.md#time-mode)), or `:copy` (for [copy mode](/replay-mode/modes.md#copy-mode)). `target` can refer to any group or pane.
