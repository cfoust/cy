...
3. The default value for [default parameters](/default-parameters.md).
...
Set the value of the [parameter](/parameters.md) at `target` for `key` to `value`. `target` must be either `:client`, a [NodeID](/api.md#nodeid) (such as `:root`), or `:persist`. `value` can be any Janet value, though `(param/set)` does enforce the type of [default parameters](/default-parameters.md).
