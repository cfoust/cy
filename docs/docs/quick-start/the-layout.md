...
`cy` has split functionality that should feel familiar to users of tmux. Like other terminal multiplexers, in `cy` you can divide your screen up into any number of views. This can be done with {{bind :root ctrl+a -}} to split the current view along a horizontal line and {{bind :root ctrl+a |}} to split along a vertical one.
...
You can remove views from your layout with {{bind :root ctrl+a x}}. However, it is important to note that unlike in other terminal multiplexers, removing a view from your layout does not kill the underlying pane; you can still attach to it again, such as by using {{bind :root ctrl+a ;}}. To remove a pane from the node tree and its view from the layout, you can use {{bind :root ctrl+a X}}.
...
1. To make the inner view fill the entire viewport, type {{bind :root ctrl+a g}}.
1. To set the width of the inner view to 80 columns, type {{bind :root ctrl+a 1}}.
