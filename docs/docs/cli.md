...
Use `test-no-context` for tests that do not need a client:

```janet
(test-no-context "no client needed"
  (assert (= 1 1)))
```

Use `expect-error` to assert that code raises an error:
...
> For this to work, you must have [enabled command detection](/command-history.md#installation) and `cy` must be installed on your system (ie available in your `$PATH`.)
